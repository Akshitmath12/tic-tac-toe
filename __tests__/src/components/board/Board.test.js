...
describe('Board Component', () => {
  ...
  test('renders the correct number of squares', () => {
    ...
    const { getAllByRole } = render(<Board squares={squares} onClick={jest.fn()} jumpTo={jest.fn()} />);
  ...
  test('calls onClick when a square is clicked', () => {
    ...
    const { getAllByRole } = render(<Board squares={squares} onClick={onClick} jumpTo={jest.fn()} />);
  ...
  test('calls jumpTo with 0 when restart button is clicked', () => {
    ...
    const { getByText } = render(<Board squares={Array(9).fill(null)} onClick={jest.fn()} jumpTo={jumpTo} />);
